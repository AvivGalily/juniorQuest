...
export class FinalHrScene extends Phaser.Scene {
  ...
  private createEmailCard(): void {
    ...
    const html = `
      ...
      ">
        <div style="
          ...
          font-weight:700;
        ">
          <span>${escapeHtml(t("finalHr.appTitle"))}</span>
        ...
        ">
          ...
          ">
            ...
            <div style="direction:${direction};text-align:${textAlign};font-size:${smallFont}px;line-height:1.35;color:#475569;font-weight:700;overflow:hidden;">
              <div>${escapeHtml(t("finalHr.from"))}</div>
              <div>${escapeHtml(t("finalHr.to"))}</div>
            </div>
          </div>
          ...
          ">${escapeHtml(t("finalHr.subject"))}</div>
          ...
          <div style="
            ...
            text-align:${textAlign};
          ">
            <p style="margin:0 0 ${Math.round(scaleY(8))}px 0;">${escapeHtml(t("finalHr.body1"))}</p>
            <p style="margin:0 0 ${Math.round(scaleY(8))}px 0;">${escapeHtml(t("finalHr.body2"))}</p>
            <p style="margin:0;">${escapeHtml(t("finalHr.body3"))}</p>
          </div>
          <div data-action="continue" style="
            ...
            text-align:center;
            cursor:pointer;
          ">${escapeHtml(t("finalHr.continue"))}</div>
        </div>
      </div>
    `;
    ...
    this.add
      ...
      .setDepth(10000)
      .on("pointerdown", () => this.continueToVictory());
  }

  private continueToVictory(): void {
    if (this.dismissed) {
      return;
    }
    this.dismissed = true;
