...
export class Level2IntroScene extends Phaser.Scene {
  ...
  private createEmailCard(): void {
    const interviewerSrc = new URL("../img/technical-interviewer-avatar.png", import.meta.url).href;
    ...
    const html = `
      ...
      ">
        <div style="
          ...
          font-weight:700;
        ">
          <span>${escapeHtml(t("level2Intro.appTitle"))}</span>
        ...
        ">
          ...
          ">
            ...
            ">
              <img src="${interviewerSrc}" alt="" style="
                width:118%;
                height:118%;
                object-fit:cover;
                display:block;
            ...
            <div style="direction:${direction};text-align:${textAlign};font-size:${smallFont}px;line-height:1.35;color:#475569;font-weight:700;overflow:hidden;">
              <div>${escapeHtml(t("level2Intro.from"))}</div>
              <div>${escapeHtml(t("level2Intro.to"))}</div>
            </div>
          </div>
          ...
          ">${escapeHtml(t("level2Intro.subject"))}</div>
          ...
          <div style="
            ...
            text-align:${textAlign};
          ">
            <p style="margin:0 0 ${Math.round(scaleY(4))}px 0;">${escapeHtml(t("level2Intro.greeting"))}</p>
            <p style="margin:0 0 ${Math.round(scaleY(4))}px 0;">${escapeHtml(t("level2Intro.body1"))}</p>
            <p style="margin:0 0 ${Math.round(scaleY(4))}px 0;">${escapeHtml(t("level2Intro.body2"))}</p>
            <p style="margin:0 0 ${Math.round(scaleY(5))}px 0;">${escapeHtml(t("level2Intro.body3"))}</p>
            <div style="
              ...
              color:#0f172a;
              font-weight:800;
            ">${escapeHtml(t("level2Intro.instructions"))}</div>
          </div>
          <div style="
