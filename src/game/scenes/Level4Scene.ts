...
export class Level4Scene extends BaseLevelScene {
  ...
  private checkpointY = scaleY(2200);
  private checkpointEvery = scaleY(450);
  private doorZone!: Phaser.GameObjects.Rectangle;
  private pushStrength = scale(220);
  ...
  create(): void {
    ...
    this.physics.world.setBounds(0, 0, this.scale.width, scaleY(2400));
    this.cameras.main.setBounds(0, 0, this.scale.width, scaleY(2400));

    this.add.rectangle(scaleX(320), scaleY(1200), this.scale.width, scaleY(2400), 0x0f1621);

    const platforms = this.physics.add.staticGroup();
    platforms.create(scaleX(320), scaleY(2360), "platform").setScale(10 * scaleX(1), 1 * scaleY(1)).refreshBody();

    let y = scaleY(2200);
    let offset = 0;
    for (let i = 0; i < 18; i += 1) {
      const x = scaleX(120 + (offset % 3) * 180);
      platforms.create(x, y, "platform").setScale(1 * scaleX(1), 1 * scaleY(1)).refreshBody();
      y -= scaleY(120);
      offset += 1;
    }

    this.player = new Player(this, scaleX(100), scaleY(2300));
    ...
    for (let i = 0; i < diff.l4.rivalsCount; i += 1) {
      const rival = new Rival(this, scaleX(180 + i * 40), scaleY(2300));
    ...
    this.doorZone = this.add.rectangle(scaleX(320), scaleY(80), scaleX(80), scaleY(40), 0x8fe388, 0.7);
  ...
  update(_: number, __: number): void {
    ...
    for (const rival of this.rivals) {
      ...
      if (this.player.y < rival.y && !rival.getData("overtaken")) {
        ...
        FloatingText.spawn(this, rival.x, rival.y - scale(20), "+100", "#8fe388");
      }
    }
    ...
    if (this.player.y > this.cameras.main.scrollY + scaleY(400)) {
      this.scoreSystem.addPenalty(-100);
      this.scoreSystem.breakCombo();
      this.applyDamage(() => {
        this.player.setPosition(scaleX(100), this.checkpointY);
      });
      FloatingText.spawn(this, this.player.x, this.player.y - scale(20), "-100", "#ff6b6b");
    }
  ...
  private completeLevel(): void {
    ...
    FloatingText.spawn(this, scaleX(320), this.cameras.main.scrollY + scaleY(80), "+2500", "#8fe388");
    this.hud.updateAll();
